...
CODE_BEB82A:		;behavior handler for BE sprites
	PHK					;$BEB82A  \
	PLB					;$BEB82B   |
...
;START OF PATCH		(don't count kremcoins as collected)
	PHA 		;pushing A just in case
	LDA !lv_sel_toggle
	BEQ .done
	PLA
	BRA CODE_BEB9D6
...
;START OF PATCH		(lui timer DK coin check hijack, don't count them as grabbed)
	PHA 		;pushing A just in case
	LDA !lv_sel_toggle
	BEQ .done
	PLA
...
CODE_BEC678:
;START OF PATCH		(don't deduct lives on death)
	LDA !lv_sel_toggle
	BEQ .done
	RTL
.done:
...
;routine to position timer sprite on top of the screen?
CODE_BEC702:
	LDY $097F				;$BEC702  \ check if timer sprite exists
...
CODE_BEC719:
; ;START OF PATCH		(lui timer hud hijack)
	JSR handle_displays

;draw timer if kong enters a screen transition or hits the goal
;you'd think this should be done in the behavior code, but nope, doesn't work from there
	LDX main_kong
	LDA $2E,x
	CMP #$0033
	BEQ .draw
...
;START OF PATCH		(lui timer end lives hijack)
 	RTS
;END OF PATCH
...
;START OF PATCH		(lui timer bypass hud face hijack)
	BRA +
;END OF PATCH
...
;START OF PATCH		(lui timer end bananas hijack)
	;RTS
;END OF PATCH
...
;haunted hall/target terror doors
CODE_BECBA0:
	LDA $0A36				;$BECBA0  \
	BIT #$0004				;$BECBA3   |
...
CODE_BED44C:
	LDY $0D5E					;$BED44C  \ index to kart sprite player is riding?
...
;lui timer includes
incsrc "lui_timer/level.asm"
incsrc "lui_timer/hud.asm"
incsrc "lui_timer/map.asm"
print pc