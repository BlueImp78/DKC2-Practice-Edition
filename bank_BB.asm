...
;maybe also check if boss beaten?
CODE_BB8114:
...
CODE_BB819F:
;START OF PATCH		(preserve timer values after skipping bonus intro screen)
	LDA !bonus_intro_started
	BEQ .done
	JSL preserve_timer_values
.done:
;END OF PATCH
...
;START OF PATCH		(enable start+select always)
	BRA CODE_BB8269
.done
;END OF PATCH
...
CODE_BB8B66:		;2P mode kong palette stuff?
	STZ $60					;$BB8B66  \
	LDA $060D				;$BB8B68   |
	CMP #$0002				;$BB8B6B   |
...
CODE_BB8B9F:
;START OF PATCH (apply selected kong palette for back kong)
	LDA !lv_sel_color
	BEQ .done
	LDA $08A4
	BEQ .dixie_front
	LDA !lv_sel_diddy_pal
	BRA +
.dixie_front:
	LDA !lv_sel_dixie_pal
+:
	RTS
.done:
;END OF PATCH
...
CODE_BB8BCA:
;START OF PATCH (apply selected kong palette for front kong)
	LDA !lv_sel_color
	BEQ .done
	LDA $08A4
	BNE .dixie_front
...
.next_payload					;	   |
	SEP #$20				;$BB8CC0   |
	LDA.w DATA_FD819A,x			;$BB8CC2   |\ If the payload data is zero, we are done uploading
	BEQ .return				;$BB8CC5   |/
...
;START OF PATCH			(play song 0F before bonus init so it doesn't crash when entering from menu)
	LDA !bonus_intro_started
	BNE .done
	LDA !lv_sel_is_bonus
	BEQ .done
	LDA !fast_retry_is_bonus		
	BNE .done		;dont play it again when restarting the bonus room
	SEP #$20
	LDA #$01
...
;START OF PATCH		(dont overwrite entrance number or animal id)
	BRA +
;END OF PATCH
...
;START OF PATCH	  (dont restart song if respawning from fast retry on normal level or bonus entered from the prac menu)
	LDY !fast_retry_is_bonus
	BNE +
	CMP current_song
	BNE .done
+:
...
CODE_BB944F:
;START OF PATCH	  (same "don't restart song on bonus retry" patch but for other version of the song)
	LDY !fast_retry_is_bonus
	BNE +
	LDA #$0003				;$BB944F  \
...
;START OF PATCH		(dont restart song if respawning from fast retry on bosses)
	CMP current_song
	BNE .done
+:
	RTS
...
;START OF PATCH		(dont restart song if respawning from fast retry on other normal levels? idk man im losing it)
	CMP current_song
	BNE .done
	RTS
.done:
...
;START OF PATCH		(set correct cutscene ID for krool 1 without the DK abuse)
	BRA CODE_BBAD2C
.done:
;END OF PATCH
...
;START OF PATCH		(fast retry after normal death animation (for castle crush warp) )
if !version == 1
	JSL fast_retry
	JSL restore_timer_values
	JSL restore_player_status
	JML CODE_B5D47A
.done:
endif
;END OF PATCH
...
; ;START OF PATCH		(fast retry on wrong warps involving kutlass)
; 	LDA level_number
; 	CMP #$0062			;check if we're in castle crush
; 	BEQ .castle_crush
; 	CMP #$0029			;or if krochead
; 	BNE +
; 	LDX main_kong
; 	LDA $06,x			;get kong X pos.
; 	CMP #$1600
; 	BCC .respawn			;if around the area where the warp happens, respawn instead
; 	BRA +


; .castle_crush:
; 	LDA $0D54			;get floor Y pos.
; 	CMP #$2AFF				
; 	BCC +

; .respawn:
; 	JSL fast_retry			;if around the general area where the wrong warp happens, respawn instead
; 	JSL restore_timer_values
; 	JSL restore_player_status
; 	JML CODE_B5D47A	
...
;START OF PATCH		(restart level when beating it)
	LDA !level_number_mirror
	STA level_number
	JSL fast_retry
...
;START OF PATCH		(preserve lui timer values and player status when changing entrances that arent 00)
	LDA $08A6
	BNE +
	LDA !level_number_mirror
	CMP level_number
...
;START OF PATCH 	(preserve lui timer and kong status on animal antics transitions)
	JSL preserve_timer_values
	JSL preserve_player_status
	STZ !fast_retry_is_bonus
.done:
...
;START OF PATCH		(restart bonus on completion if entered from menu, preserve stuff)
	LDA !lv_sel_is_bonus
	BEQ +
	LDA !level_number_mirror
	STA level_number
	STZ !timer_started
	JSR set_player_items			;clear coin counts	
	JSL fast_retry
	JSL restore_timer_values
	JSL restore_player_status
	JML CODE_B5D47A
...
;START OF PATCH		(same as above but for 1.0
	LDA !lv_sel_is_bonus
	BEQ +
	LDA !level_number_mirror
	STA level_number
...
;START OF PATCH		(restart boss level when beaten)
	LDA !level_number_mirror
	STA level_number
	JSL fast_retry
	JSL restore_menu_player_status
...
;START OF PATCH (add label for CODE_BBC0A4 wrapper)
CODE_BBC0A0:
;END OF PATCH
...
; ;START OF PATCH		(fast retry on successful wrong warps with barrels or cannons)
; 	JSL fast_retry
; 	JSL restore_timer_values
; 	JSL restore_player_status
; 	JML CODE_B5D47A
...
;START OF PATCH		(save selected menu options to sram)
	LDY #$00E8+$0B	
	LDX #$000B
.copy:
	LDA.l $1A04,x
...
;START OF PATCH		(load menu options from sram)
	LDY #$00E8+$0B
	LDX #$000B
.copy:
	LDA [$D9],y
...
;level select includes
incsrc "level_select/code/level_select_routines.asm"
incsrc "level_select/code/fast_retry.asm"
print pc
