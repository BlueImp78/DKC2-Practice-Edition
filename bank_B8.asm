...
;START OF PATCH		(don't play bonus victory song on rickety or haunted if entered from pac menu)
	LDA !lv_sel_is_bonus
	BNE .done
;END OF PATCH
...
;START OF PATCH		(fast retry on timeout in bonuses)
	JSL fast_retry
	RTS
.done:
;END OF PATCH
...
CODE_B883B8:					;	   |
	...
	LDA #$0003				;$B883CA   |
	STA $0A36				;$B883CD   |
;START OF PATCH		(fast retry with kong)
	LDA !lv_sel_toggle
	BEQ .done
	JSL fast_retry
	RTS
...
;START OF PATCH		(no music transition on normal death animation (for castle crush warp) )
if !version == 1
	LDA !lv_sel_toggle
	BEQ .done
	BRA CODE_B8841B
...
CODE_B88893:
;START OF PATCH		(don't play level end song when hitting goal)
	BRA CODE_B8889A
;END OF PATCH
...
;START OF PATCH		(fast retry on hit/pits in bonuses)
	JSL fast_retry
	RTS
.done:
;END OF PATCH
...
kong_behavior_table:
	...
	dw kong_behavior_05 : db $01, $00	;05	;death?
...
;START OF PATCH		(custom kong behaviour for fast retry)
	dw kong_behavior_80 : db $00, $00	;80
;END OF PATCH
...
if !version == 1	
	LDA level_number
	CMP #$0062	;check if in CC
	BNE +
	LDA $0D54	;get floor Y pos
	CMP #$1A55	;check if in second half of stage
	BCC +
	CMP #$2D00	;check if in general area where the wrong warp happens
	BCS +
	JSR CODE_B8994C
	JMP CODE_B8996E
...
;START OF PATCH		(fast retry with animal)
	LDA !lv_sel_toggle
	BEQ .done
	JSL fast_retry
	JMP CODE_B8996E
...
kong_behavior_47:
;START OF PATCH		(clear song number so the level song restarts when the level does too)
	STZ current_song
;END OF PATCH
	LDA #$0008				;$B8ABE6  \
	TSB $08C2				;$B8ABE9   |
...
;START OF PATCH			(makes bosses always drop the kremcoin)
	PHA			;prob not needed but preserving A here just in case
	LDA !lv_sel_toggle
	BEQ .done
	PLA
	BRA +
...
;START OF PATCH		(custom kong behavior for fast retry)
kong_behavior_80:
	; LDA !fast_retry_is_bonus
	; BNE .shit		
	LDA level_number		;if level number doesn't match the mirror, then we are in a sub screen
	CMP !level_number_mirror
	BNE +				;if so, dont turn on the initial timer delay
	LDA $08A6
	CMP #$0002			;same if entrance is not start of level or checkpoint
	BCS +
.shit:
        STZ !timer_started
+:
