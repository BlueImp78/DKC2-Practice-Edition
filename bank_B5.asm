...
;START OF PATCH		(no song transition if silence selected in jukebox)
	LDX !lv_sel_song
	BEQ +
;END OF PATCH
...
;START OF PATCH		(default to stereo)
	LDA #$0000
;END OF PATCH
...
CODE_B5D424:					;	   |
;START OF PATCH		(lui timer map hijack)
	LDA !lv_sel_toggle
	BEQ .done
	JSL every_map_frame
.done:
...
CODE_B5D47A:
; ;START OF PATCH		(preserve lui timer values)
...
;START OF PATCH		(routine to mute in-game music if silence selected on jukebox)
mute_music:
	LDA !lv_sel_song
	BNE .done
	LDA #$0401
