...
;START OF PATCH		(run practice menu right away on boot)
	JML level_select_init
;;END OF PATCH
...
;START OF PATCH		(lui timer level hijack)
	LDA !lv_sel_toggle
	BEQ .done
	JSL every_igt_frame
.done:
...
; ;START OF PATCH		(call routine to mute music if silence selected in prac menu jukebox, post NMI)
; 	LDA !already_muted
; 	BNE +
; 	JSL mute_music
; 	LDA #$0001
...
;START OF PATCH		(draw timer on pause, respawn at chosen entrance if L pressed, at current if R pressed)
	LDA !pause_timer_started
	BNE +
	JSL draw_timer_global
	INC !pause_timer_started
+:
	...
	STA level_number
.respawn:
	JSR unpause_without_sound		;force unpause and respawn

	LDA !lv_sel_is_bonus			;if in a bonus from menu, always respawn at current
	BEQ ++
	STZ !pause_L_pressed
++:
	JSL fast_retry
...
;START OF PATCH		(go to prac menu on start+select)
	LDA !lv_sel_toggle
	BEQ .done
	JML level_select_init
.done:
...
;START OF PATCH		(add label to JSR to)
unpause_without_sound:
;END OF PATCH
	LDA #$00FF				;$808B09   |
	TRB $0621				;$808B0C   |
	STZ $19B0				;$808B0F   |
...
;START OF PATCH		(clear pause timer var)
	PHP
	LDA !lv_sel_toggle
	BEQ .done
	STZ !pause_timer_started
.done:
	PLP
;END OF PATCH
	RTS					;$808B18  /
...
;START OF PATCH		(lui timer bonus intro hijack)
	LDA #$0001
	STA !bonus_intro_started
...
CODE_80973E:					;	   |
	...
	STA $7E8968,x				;$80978A   |
CODE_80978E:					;	   |
;START OF PATCH		(allow intro bypass always)
	LDA #$FFFF
	STA.l enable_intro_bypass
;END OF PATCH
...
CODE_8097CD:		;init game mode select?
...
CODE_809A13:	;game mode screen controller handler?
...
DATA_809C99:
;START OF PATCH		(add label for text for song 00)
	dw silence
	;dw !null_pointer
;END OF PATCH
	dw DATA_809CD9
...
;START OF PATCH		(add text for song 00)
silence:
	db "SILENCE", $00
;END OF PATCH
...
;START OF PATCH (check if L and R are pressed)
	...
	BEQ .frame_finish_trampoline		;	   |/ If so, skip setting transition action.
...
;END OF PATCH
	...
	BNE .return_2_game			;$80AB68   |/ then we are returning to the game
	JMP .load_selected_file			;$80AB6A  / Otherwise load the file that is selected
...
incsrc "level_select/code/level_select_main.asm"
